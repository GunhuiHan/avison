...
class _CalendarPageState extends State<CalendarPage>
  ...
  void initState() {
    ...
    for (int i = 0; i < widget.programs.length; i++) {
      for (int j = 0; j < widget.programs[i].date.length; j++) {
        _events.update(
          widget.programs[i].date[j],
          (val) {
            List newList = val;
            newList.add(widget.programs[i]);
            return newList;
          },
          ifAbsent: () => [widget.programs[i]],
        );
      }
    }
